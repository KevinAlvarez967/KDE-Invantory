const API_CUENTA = '../../Core/api/Empleado/Usuario.php?action=';
...
function readOneUsuario(id)
{
    ...
    .done(function( response ) {
        if ( response.status ) {
            ...
            $( '#idusuario' ).text( response.dataset.id_usuario);
...
function openModalProfile()
{
    ...
    .done(function( response ) {
        if ( response.status ) {          
            $( '#Nombres1' ).val( response.dataset.nombre);
            $( '#Apellidos1' ).val(response.dataset.apellido)
            $( '#patata' ).val( response.dataset.correo);
